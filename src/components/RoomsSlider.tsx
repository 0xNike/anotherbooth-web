 ...
 *  GSAP transition (no native scroll). At the first/last room the next
 *  gesture releases — the page scrolls on to the hero / How It Works.
 *
 *  Fallback: reduced motion or no JS → `gsap-active` is never added, so the
 *  rooms stay as plain stacked, natively-scrolling panels. */
export default function RoomsSlider() {
  ...
  useGSAP(
    () => {
      ...
      // Show exactly one room instantly (used on enter / re-enter).
      ...
      // Animated transition from the current room to room `i`.
      ...
      // Hand control back to the page: scroll to the next section (down) or
      // back above the slider (up). Targeting elements is more reliable than
      // the pin's computed end.
      ...
      const observer = Observer.create({
        ...
        wheelSpeed: -1, // align wheel direction with touch (down = next)
        tolerance: 10,
        preventDefault: true,
        onUp: () => {
          if (animating) return;
